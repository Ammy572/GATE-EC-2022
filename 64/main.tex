...
\solution\\
On crossing between $rr$ and $RR$ we get:
\begin{table}[h]
...
F_1 &= Rr,Rr,Rr,Rr
\end{align}
When $F_1$ ($Rr$) is test-crossed with $rr$ we get:
\begin{table}[h]
	\input{/home/aman123/GATE-EC(2022)/XL/64/tables/table2.tex}
	\caption{Table2: Crossing btw $Rr$ and $rr$}
	\label{tab: Table2}
\end{table}\\
Now, Let $X$ be the random variable representing the gene.
\begin{align}
X &= \begin{cases}
1 &  R\\
0 &  r
\end{cases}
\end{align}
Probability that the population is male fertile is given by:
\begin{align}
p_X(k) &= \comb{1}{k} \brak{\frac{1}{2}}^k \brak{\frac{1}{2}}^{n-k}\\
p_X(1) &= \frac{1}{2}
\end{align}
